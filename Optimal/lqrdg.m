function [k,s,e,Qd,Nd,Rd] = lqrdg(a,b,q,r,nn,Ts)
...
if any(size(q)~=Nx),
    ctrlMsgUtils.error('Control:design:lqr2','lqrdg','A','Q')
elseif any(size(r)~=Nu),
    ctrlMsgUtils.error('Control:design:lqr3','lqrdg')
elseif ~isequal(size(nn),[Nx Nu]),
    ctrlMsgUtils.error('Control:design:lqr2','lqrdg','B','N')
% elseif norm(q'-q,1) > 100*eps*norm(q,1),
%     ctrlMsgUtils.warning('Control:design:MakeSymmetric','lqrdg(...,Q,R,Ts)','Q','Q','Q')
% elseif norm(r'-r,1) > 100*eps*norm(r,1),
%     ctrlMsgUtils.warning('Control:design:MakeSymmetric','lqrdg(...,Q,R,Ts)','R','R','R')
end
...
%     ctrlMsgUtils.warning('Control:design:MustBePositiveDefinite','[Q N;N'' R]','lqrdg')
end
